...
'providers' => [
    // ...
    MalaveHaxiel\Html\HtmlServiceProvider::class,
    // ...
],
```

3. Also, you need to register in the `app/Http/Kernel.php` file the `\MalaveHaxiel\Html\Alert\Middleware::class` middleware **BEFORE** the `EncryptCookies` middleware, for Laravel 5.8 and later the middleware needs to be registered **AFTER** the `StartSession` middleware. For Laravel 5.4 and later, it's in the `$middlewareGroups` array and for previous versions (Laravel 5.3, 5.2, 5.1) it's in the `$middleware` array:
...
Field => MalaveHaxiel\Html\Facades\Field
...
'aliases' => [
    // ...
    'Access' => MalaveHaxiel\Html\Facades\Access::class,
    // ...
],
```

Optionally, you may also run `php artisan vendor:publish --provider='MalaveHaxiel\Html\HtmlServiceProvider'` to publish the configuration file in `config/html.php` and review its options and values.
...
The MalaveHaxiel\Html package is open-sourced software licensed under the MIT license.
